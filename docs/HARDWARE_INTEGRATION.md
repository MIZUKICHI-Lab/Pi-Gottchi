...
睡眠時はLive WebSocketと連続録音を停止し、LCDバックライトを `SLEEP_BACKLIGHT` まで下げ、サーボを睡眠姿勢へ移します。Pi Zero WのバックライトはON/OFF式なので、既定では点灯した睡眠顔を維持します。消灯する場合だけ明示的に0を設定します。PWM対応機では既定12%まで減光します。ボタン、揺れ、充電開始、一定以上の環境音で起床し、画面とLiveを戻します。環境音の確認は3秒ごとの短い録音なので、確実な起床手段はボタンまたはIMUです。
