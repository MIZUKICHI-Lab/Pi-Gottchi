...
Gemini側は `START_OF_ACTIVITY_INTERRUPTS` を使い、返答音声の再生が始まる直前に新しい発話が届いた場合、古い返答の後へ持ち越さないようにします。内蔵AECがないため、実際のスピーカー再生中だけは従来どおりマイク送信を止めます。返答も原則1短文に制限し、半二重で聞けない時間を短くします。
