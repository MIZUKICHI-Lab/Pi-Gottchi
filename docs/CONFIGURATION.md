...
| `STARTUP_AWAKE_SEC` | いいえ | `0` | 起動後に自動睡眠を禁止する時間（0〜1800秒） |
| `AUTO_SLEEP_SEC` | いいえ | `20` | 通常時の自動睡眠までの無操作時間（5〜86400秒） |
| `NIGHT_AUTO_SLEEP_SEC` | いいえ | `10` | 夜間の自動睡眠までの無操作時間（5〜86400秒） |

Pi Zero WでWhisplayドライバが `Simple Switch` と判定した場合、1〜100はすべて同じ点灯状態です。既定では睡眠顔を表示したままにし、明示的に消灯したい場合だけ `SLEEP_BACKLIGHT=0` を設定します。PWM対応機では12%減光を使います。自動睡眠は見た目とサーボ姿勢だけを休ませ、Liveの会話待機は継続します。マイクを止める深い睡眠は「おやすみ」「さよなら」等の明示発話だけです。
